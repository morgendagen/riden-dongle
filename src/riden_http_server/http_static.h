...
static const char HTML_HEADER[] PROGMEM =
    ...
    "              cursor: pointer;"
    "            }"
    "            .control input[type='text'] {font-family: 'Noto Sans Mono', monospace; font-size: 1.2em; text-align:right;}"
    "        </style>"
    "    </head>"
    "    <body>"
...
static const char HTML_CONTROL_BODY[] PROGMEM = 
    ...
    "  <h2>Power Supply Control</h2><div>"
    "  <table class='control'>"
    "    <tbody>"
    "      <colgroup><col width='20%' /><col width='1%' /><col width='1%' /><col width='20%' /></colgroup>"
    "      <tr><th>Set</th><th></th><th>Output</th><th></th></tr>"
    "      <tr>"
    "        <td>"
    "          <input type='text' id='set_v' size='6' value='' />"
    "        </td>"
    "        <td>"
    ...
    "      <tr>"
    "        <td>"
    "          <input type='text' id='set_c' size='6' value='' />"
    "        </td>"
    "        <td>"
    ...
    "    self.out_v.innerHTML = obj.out_v.toFixed(3).padStart(6, ' ') + ' V';"
    "    self.out_i.innerHTML = obj.out_c.toFixed(3).padStart(6, ' ') + ' A';"
    "    if (full_set) {"
    "      self.set_v.value = obj.set_v.toFixed(3);"
    "      self.set_c.value = obj.set_c.toFixed(3);"
    "    }"
    "    if (obj.out_on) {"
    ...
    "    }"
    "    if (obj.ext_t_c === null) { self.ext_c.innerHTML = '  --.-'; } else {self.ext_c.innerHTML = obj.ext_t_c.toFixed(1).padStart(6, ' ');}"
    "    self.int_c.innerHTML = obj.int_t_c.toFixed(1).padStart(6, ' ');"    
    "    self.batt_v.innerHTML = obj.batt_v.toFixed(3).padStart(6, ' ');"    
    "    let img = 'nobatt';"
    "    if (obj.prot === 'OVP') { img = 'ovp'; }"
    "    else if (obj.prot === 'OCP') { img = 'ocp'; }"
    "    else if (obj.batt_mode) { img = 'batt'; }"    
    ...
    "    try {"
    "      fetch('/toggle_out').then((response) => {"
    "          document.body.style.cursor = 'default';"
